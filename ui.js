...
// === Funções para Manutenções (API) ===
async function carregarManutencoes(veiculoId) {
    try {
        const resp = await fetch(`/api/veiculos/${veiculoId}/manutencoes`);
        if (!resp.ok) throw new Error('Erro ao buscar manutenções');
        const manutencoes = await resp.json();
        atualizarListaManutencoes(manutencoes);
    } catch (err) {
        exibirNotificacao('Erro ao carregar manutenções: ' + err.message, 'erro');
    }
}

function atualizarListaManutencoes(manutencoes) {
    if (!listaHistoricoElement) return;
    listaHistoricoElement.innerHTML = '';
    if (!manutencoes || manutencoes.length === 0) {
        listaHistoricoElement.innerHTML = '<li>Nenhum histórico registrado.</li>';
        return;
    }
    manutencoes.forEach(m => {
        const li = document.createElement('li');
        li.innerHTML = `<strong>${m.descricaoServico}</strong> - Data: ${new Date(m.data).toLocaleDateString()}<br>Custo: R$ ${m.custo.toFixed(2)}${m.quilometragem ? `<br>Quilometragem: ${m.quilometragem} km` : ''}`;
...
async function adicionarManutencao(veiculoId, dadosFormulario) {
    try {
        ...
        if (!resp.ok) {
            ...
            throw new Error(erro.error || 'Erro ao adicionar manutenção');
        }
        exibirNotificacao('Manutenção adicionada com sucesso!', 'sucesso');
        carregarManutencoes(veiculoId);
    } catch (err) {
        exibirNotificacao('Erro ao adicionar manutenção: ' + err.message, 'erro');
    }
}
...
function exibirDetalhesCompletos(veiculo) {
    ...
    // Chama a função para buscar manutenções do backend
    if (veiculo && veiculo._id) {
        carregarManutencoes(veiculo._id);
    }
